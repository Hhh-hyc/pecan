...
model2netcdf.FATES <- function(outdir,sitelat,sitelon,start_date,end_date,pfts,settings = NULL,process_partial = FALSE){

model2netcdf.FATES <- function(outdir, sitelat, sitelon, pfts, matched_var) {
  ...
  var_update <- function(out,oldname,newname,newunits=NULL,long_name=NULL){
    if (oldname %in% ncin_names) {
      ...
      ## check pft dimension
      if any(grep('pft',nc.get.dim.names(ncin, oldname))){ # include fates_levpft
        dimension <- xypt
      }else{ # only xyt
        dimension <- xyt
      }
      ## transpose dimensions into (,t)
      if (nc.get.dim.names(ncin,oldname)[length(nc.get.dim.names(ncin,oldname))]=='time'){
            dat <- ncdf4::ncvar_get(ncin,oldname, start=c(1,1,1)) # time at the tail of dims
  ...
  for (year in unique(years)) {
      ...
      ncout <- ncdf4::nc_open(oname, write = TRUE) # PEcAn yearly file
      time_points_year <-c()
      out <- NULL
      for (month in ysel){
        month_file <- files[ysel[month]] 
        nc_month <- ncdf4::nc_open(month_file) # read monthly file
        ## Create time bounds to populate time_bounds variable iteratively
        var_bound <- ncvar_get(nc_month, "time_bounds") # start,end day of month
        ...
        for (name_param in matched_var){out <- var_update(out,name_param[1],name_param[2],name_param[3],name_param[4])} # convert monthly fates output into one variable
        ...
        for (i in seq_along(out$var)) {ncdf4::ncvar_put(ncout, out$var[[i]], out$dat[[i]], start(1,1,month))} # write monthly converted variable into PEcAn output
      }
      ## put in time_bounds after writing all th months
      t_new <- ncdf4::ncdim_def(name = "time", units = paste0("days since ", init_year, "-01-01 00:00:00"),
                      vals = as.vector(time_points_year), calendar = "noleap", unlim = TRUE)
